...
uploaded_file = st.sidebar.file_uploader("최근10년간 국내지진목록.xlsx 파일을 업로드하세요", type=["xlsx"])
...
filtered_df = filtered_df[
    (filtered_df['규모'] >= 규모_선택[0]) &
    (filtered_df['규모'] <= 규모_선택[1])
]
...
if {'위도', '경도'}.issubset(filtered_df.columns):
    ...
    st.warning("⚠️ 지도 표시를 위해 '위도', '경도' 컬럼이 필요합니다.")
...
# 3️⃣ 규모 구간별 연도별 추세
# ----------------------
st.header("3️⃣ 규모 구간별 연도별 지진 발생 변화")
...
# 데이터 보기
# ----------------------
with st.expander("📄 필터 적용된 원본 데이터 보기"):
